...
import { Role, Transcript, UltravoxExperimentalMessageEvent, UltravoxSession, UltravoxSessionStatus } from 'ultravox-client';
import { getSlotsTools, bookSlotTool } from '@/lib/clientTools';
...
export async function startCall(callbacks: CallCallbacks, callConfig: CallConfig, showDebugMessages?: boolean): Promise<void> {
  ...
  if (!joinUrl && !uvSession) {
    ...
    // Register our dental appointment tools
    uvSession.registerToolImplementation("get_slots", getSlotsTools);
    uvSession.registerToolImplementation("book_slot", bookSlotTool);
